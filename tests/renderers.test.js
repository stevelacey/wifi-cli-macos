...
vi.mock('qrcode-terminal', () => ({
  default: {
    generate: vi.fn((uri, opts, cb) => cb('MOCK_QR_OUTPUT')),
  },
}))
...
describe('formatHelp', () => {
  ...
  it('renders subcommand help with description', () => {
    const cmd = makeCmd({ name: 'connect', usage: '[network]', description: 'Connect to a network', parent: {} })
    const helper = makeHelper({ opts: [{ flags: '--verbose', description: 'Verbose output' }] })
  ...
  it('renders subcommand help without description', () => {
    const cmd = makeCmd({ name: 'restart', usage: '', description: '', parent: {} })
    const helper = makeHelper({ opts: [{ flags: '--help', description: 'Show help' }] })
    const result = strip(formatHelp(cmd, helper))
  ...
  it('renders root help with commands sorted alphabetically', () => {
    const cmds = [
      makeCmd({ name: 'list', description: 'List networks' }),
      makeCmd({ name: 'connect', description: 'Connect' }),
    ]
...
describe('renderNetwork', () => {
  ...
  it('handles missing band and security', () => {
    ...
    const result = strip(renderNetwork(nets[0], nets))
    expect(result).toContain('Open')
  })

  it('includes band and security in output', () => {
    const result = strip(renderNetwork(networks[0], networks))
    expect(result).toContain('5 GHz')
    expect(result).toContain('WPA2')
  })

  it('includes ssid in output', () => {
    expect(strip(renderNetwork(networks[0], networks))).toContain('HomeNet')
  })

  it('pads ssid to max length across networks', () => {
    const maxLen = 'LongerNetworkName'.length
    expect(strip(renderNetwork(networks[0], networks)).startsWith('HomeNet'.padEnd(maxLen))).toBe(true)
    expect(strip(renderNetwork(networks[1], networks)).startsWith('LongerNetworkName')).toBe(true)
  })

  it('selects max ssid length when first network is longer', () => {
    const reversed = [networks[1], networks[0]]
    const result = strip(renderNetwork(networks[0], reversed))
...
describe('renderQr', () => {
  it('escapes special characters in ssid and password', () => {
    let capturedUri
    qrcode.generate.mockImplementation((uri, opts, cb) => { capturedUri = uri; cb('QR') })
    renderQr('Net;Name', 'pass"word')
...
describe('subcommandTerm', () => {
  ...
  it('appends optional args in square brackets', () => {
    const cmd = makeCmd({ name: 'connect', args: [{ n: 'network', required: false }] })
    expect(strip(subcommandTerm(cmd))).toBe('connect [network]')
  })

  it('appends required args in angle brackets', () => {
    const cmd = makeCmd({ name: 'connect', args: [{ n: 'network', required: true }] })
    expect(strip(subcommandTerm(cmd))).toBe('connect <network>')
  })

  it('appends variadic marker to optional arg', () => {
    const cmd = makeCmd({ name: 'dns', args: [{ n: 'servers', required: false, variadic: true }] })
    expect(strip(subcommandTerm(cmd))).toBe('dns [servers...]')
  })

  it('appends variadic marker to required arg', () => {
    const cmd = makeCmd({ name: 'cmd', args: [{ n: 'items', required: true, variadic: true }] })
