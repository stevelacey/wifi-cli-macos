...
vi.mock('@clack/core', () => {
  const AutocompletePrompt = vi.fn(function(opts) { this._opts = opts })
  AutocompletePrompt.prototype.prompt = vi.fn(async function() { return null })
...
describe('multiselect', () => {
  ...
  describe('render', () => {
    ...
    it('renders submit state with selected values', async () => {
      await multiselect(opts)
      const result = strip(getRender().call({ ...baseCtx, state: 'submit', selectedValues: ['a'] }))
    ...
    it('renders error state', async () => {
      await multiselect(opts)
      const ctx = { ...baseCtx, state: 'error', userInput: 'x', filteredOptions: [options[0]], error: 'Required' }
      expect(strip(getRender().call(ctx))).toContain('Required')
    })

    it('renders match count when filtering', async () => {
      await multiselect(opts)
      const ctx = { ...baseCtx, userInput: 'Net', userInputWithCursor: 'Net|', filteredOptions: [options[0]] }
      expect(strip(getRender().call(ctx))).toContain('1 match')
    })

    it('renders plural matches', async () => {
      await multiselect(opts)
      const ctx = { ...baseCtx, userInput: 'Net', userInputWithCursor: 'Net|', filteredOptions: options }
    ...
    it('uses value as label when option has no label', async () => {
      const noLabelOpts = { message: 'Pick', options: [{ value: 'raw-value' }, { value: undefined }] }
      await multiselect(noLabelOpts)
      const ctx = { ...baseCtx, options: noLabelOpts.options, filteredOptions: noLabelOpts.options, state: 'submit', selectedValues: ['raw-value', undefined] }
      const result = strip(getRender().call(ctx))
