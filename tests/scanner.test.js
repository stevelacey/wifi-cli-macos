...
describe('connect', () => {
  it('omits falsy password', () => {
    connect('Net', null)
    expect(spawn).toHaveBeenCalledWith(BIN, ['connect', 'Net'], { stdio: 'pipe' })
  })

  it('omits password when not provided', () => {
    connect('OpenNetwork')
    expect(spawn).toHaveBeenCalledWith(BIN, ['connect', 'OpenNetwork'], { stdio: 'pipe' })
  })

  it('spawns with connect command and ssid+password', () => {
    connect('MyNetwork', 'secret')
    expect(spawn).toHaveBeenCalledWith(BIN, ['connect', 'MyNetwork', 'secret'], { stdio: 'pipe' })
  })
})
...
describe('current', () => {
  ...
  it('returns trimmed SSID from execSync', () => {
    ...
    expect(execSync).toHaveBeenCalledWith(`"${BIN}" current`, { timeout: 5000 })
  })
})
...
describe('disconnect', () => {
  it('calls spawnSync with disconnect command', () => {
    disconnect()
    expect(spawnSync).toHaveBeenCalledWith(BIN, ['disconnect'], { stdio: 'pipe' })
  })
})
...
describe('forget', () => {
  it('calls spawnSync with forget and ssid', () => {
    forget('OldNetwork')
    expect(spawnSync).toHaveBeenCalledWith(BIN, ['forget', 'OldNetwork'], { stdio: 'pipe' })
  })
})
