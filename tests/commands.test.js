...
beforeEach(() => {
  ...
  scanner.scan.mockImplementation((cb) => {
    cb(
      null,
      JSON.stringify({
        networks: [
  ...
  wifi.findPassword.mockReturnValue("secret-password")
  wifi.getSavedNetworks.mockReturnValue(["HomeNet", "WorkNet"])
  wifi.isPrivateRelay.mockReturnValue(false)
  wifi.networkQuality.mockReturnValue({
    stdout: { on: vi.fn() },
    on: vi.fn(),
  })
  wifi.off.mockReturnValue(true)
  wifi.on.mockReturnValue(true)
...
describe("connect", () => {
  ...
  it("connects without writing when no keychain password found", async () => {
    prompts.select.mockResolvedValueOnce("WorkNet")
    prompts.password.mockResolvedValueOnce("")
    wifi.findPassword.mockReturnValueOnce("")
    scanner.connect.mockReturnValue({
      on: (e, cb) => {
        if (e === "close") cb(0)
      },
  ...
  it("retries connection when retry is enabled", async () => {
    vi.useFakeTimers()
    scanner.scan.mockImplementationOnce((cb) => {
      cb(
        null,
        JSON.stringify({
...
describe("forget", () => {
  ...
  it("shows no saved networks when list is empty", async () => {
    wifi.getSavedNetworks.mockReturnValue([])
    await run("forget", undefined)
...
describe("mac", () => {
  ...
  it("rejects MAC change when private relay is on", async () => {
    wifi.isPrivateRelay.mockReturnValue(true)
...
describe("off", () => {
  it("turns Wi-Fi off", async () => {
    await run("off")
    expect(wifi.off).toHaveBeenCalled()
  })
})
...
describe("on", () => {
  it("turns Wi-Fi on", async () => {
    await run("on")
    expect(wifi.on).toHaveBeenCalled()
  })
})
...
describe("qr", () => {
  ...
  it("omits password line when no password found", async () => {
    wifi.findPassword.mockReturnValue("")
    await run("qr")
...
describe("restart", () => {
  it("restarts Wi-Fi", async () => {
    await run("restart")
    expect(wifi.restart).toHaveBeenCalled()
  })
})
...
describe("test", () => {
  it("displays speed result on success", async () => {
    const proc = {
      stdout: {
        on: vi.fn((e, cb) => {
          if (e === "data") cb('{"dl_throughput":100000000,"ul_throughput":50000000,"base_rtt":20}')
        }),
      },
      on: vi.fn((e, cb) => {
        if (e === "close") cb(0)
    ...
    wifi.networkQuality.mockReturnValue(proc)
    await run("test")
  ...
  it("omits latency when base_rtt is absent", async () => {
    const proc = {
      stdout: {
        on: vi.fn((e, cb) => {
          if (e === "data") cb('{"dl_throughput":100000000,"ul_throughput":50000000}')
        }),
      },
      on: vi.fn((e, cb) => {
        if (e === "close") cb(0)
