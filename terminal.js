...
export const formatBars = (rssi) => {
  const bars = "▁▂▃▄▅▆▇"
  const n = [-55, -60, -67, -70, -75, -80, -90].filter((t) => rssi > t).length
  const color = rssi > -67 ? "green" : rssi > -70 ? "yellow" : "red"
  const filled = bars.slice(0, n)
  const empty = bars.slice(n)
  return {
    color,
    signal: isTTY() ? filled[color] + empty.dim.grey : filled.padEnd(bars.length),
  }
}
...
export const formatHelp = (cmd, helper) => {
  if (cmd.parent) {
    ...
    return [`${"Usage:".yellow} wifi ${cmd.name()} ${cmd.usage().cyan}`, "", ...(desc ? [desc.white, ""] : []), "Options:".yellow, optLines, ""].join("\n")
  }
  ...
  const cmdLines = cmds
    ...
    .join("\n")
  return [name, "", "Usage:".yellow, `  wifi ${"[command]".white}`, "", "Commands:".yellow, cmdLines, ""].join("\n")
}
...
export const formatNetwork = ({ ssid, rssi, security, band }, networks) => {
  const max = networks.reduce((a, b) => (a.ssid.length > b.ssid.length ? a : b)).ssid.length
  const { signal, color } = formatBars(rssi)
  const details = `${(band || "").padEnd(9)} ${(security || "").padEnd(6)}`
  return `${ssid.padEnd(max)[color]}  ${signal}  ${details.grey}`
}

export const formatQr = (ssid, pass) => {
  const escapeUri = (s) => s.replace(/([\\;,"])/g, "\\$1")
  const uri = pass ? `WIFI:T:WPA;S:${escapeUri(ssid)};P:${escapeUri(pass)};;` : `WIFI:T:nopass;S:${escapeUri(ssid)};;`
  let code
...
export const print = (...args) => console.log(...args.map((a) => (typeof a === "string" && !isTTY() ? colors.strip(a) : a)))
