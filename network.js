...
export const getDhcpRouter = () => {
  ...
  return match?.[1].split(",").map((s) => s.trim())[0] ?? ""
}
...
export const getRouter = () => {
  return (
    tryRun(`route -n get default | awk '/gateway/{print $2}'`) || tryRun(`networksetup -getinfo Wi-Fi | awk -F': ' '/^Router/{print $2}'`) || getDhcpRouter()
  )
}
