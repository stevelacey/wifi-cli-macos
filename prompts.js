...
import { isCancel, limitOptions, password, S_BAR, S_BAR_END, S_CHECKBOX_ACTIVE, S_CHECKBOX_INACTIVE, S_CHECKBOX_SELECTED, S_RADIO_ACTIVE, S_RADIO_INACTIVE, spinner, symbol } from '@clack/prompts'
...
const select = (opts) => new AutocompletePrompt({
  ...
  render() {
    ...
    const s = [`${symbol(this.state)}  ${message}`]

    switch (this.state) {
      case 'submit': {
        const selected = this.selectedValues.map(v => opts.options.find(o => o.value === v)).filter(Boolean).map(label)
        return `${s[0]}\n${S_BAR.grey}${selected.length ? `  ${selected.join(', ').grey}` : ''}`
      }
      case 'cancel':
        return `${s[0]}\n${S_BAR.grey}`
      default: {
        ...
        const items = limitOptions({
          ...
          style: (opt, active) => {
            ...
            return active ? `${S_RADIO_ACTIVE.green} ${l}${hint}` : `${S_RADIO_INACTIVE.grey} ${l.grey}`
          },
          maxItems: opts.maxItems ?? 20,
          output: opts.output,
        })

        return [...s, ...items.map(i => `${bar}${i}`), S_BAR_END.cyan].join('\n')
      }
    }
  },
}).prompt()

const multiselect = (opts) => new AutocompletePrompt({
  options: opts.options,
  multiple: true,
  filter: opts.filter ?? defaultFilter,
  signal: opts.signal,
  ...
  render() {
    ...
    const matchCount = this.filteredOptions.length !== this.options.length
      ? ` (${this.filteredOptions.length} match${this.filteredOptions.length === 1 ? '' : 'es'})`.grey : ''
    const selCount = this.selectedValues.length > 0
      ? ` (${this.selectedValues.length} selected)`.grey : ''
      ...
      default: {
        ...
        const items = limitOptions({
          ...
          style: (opt, active) => {
            ...
            if (selected) return `${S_CHECKBOX_SELECTED.green} ${l}`
            return active ? `${S_CHECKBOX_ACTIVE.grey} ${l}` : `${S_CHECKBOX_INACTIVE.grey} ${l.grey}`
          },
          maxItems: opts.maxItems ?? 20,
          output: opts.output,
        })
