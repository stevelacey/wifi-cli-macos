...
export const check = () => {
  ...
  const granted = (() => {
    try {
      return execSync(`"${bin}" check`, { timeout: 5000 }).toString().trim() === "granted"
    } catch {
      return false
    }
  })()
  if (!granted) {
    try {
      execSync(`"${bin}" request-permission`, { timeout: 30000 })
    } catch {
...
export const current = () => {
  try {
    return execSync(`"${bin}" current`, { timeout: 5000 }).toString().trim()
  } catch {
    return ""
  }
}
